	...
	echo '<table class="form-table">';
		...
		echo '<tr class="sc_meta_box_row">';
			...
			echo '<td class="sc_meta_box_td" colspan="4">';
				...
				echo '<input type="text" class="sc_datepicker" name="sc_event_date" value="' . $date . '" placeholder="mm/dd/yyyy"/>';
				echo '<span class="sc_recurring_help description"' . $recurring_display . '>&nbsp;' . __('This is the first day that this event occurs', 'pippin_sc') . '</span>';
			echo '</td>';
		...
		echo '<tr class="sc_meta_box_row">';
			...
			echo '<td class="sc_meta_box_td" colspan="4">';
				echo '<input type="text" class="small-text" name="sc_event_time_hour" value="' . $hour . '"/>';
				echo '<span class="sc_event_time_separator">&nbsp;:&nbsp;</span>';
				echo '<input type="text" class="small-text" name="sc_event_time_minute" value="' . $minute . '"/>';
				...
				echo '<input type="text" class="small-text" name="sc_event_end_time_hour" value="' . $end_hour . '"/>';
				echo '<span class="sc_event_time_separator">&nbsp;:&nbsp;</span>';
				echo '<input type="text" class="small-text" name="sc_event_end_time_minute" value="' . $end_minute . '"/>';
		...
		echo '<tr class="sc_meta_box_row">';
			...
			echo '<td class="sc_meta_box_td" colspan="2"><label for="sc_event_date">' . __('Recurring', 'pippin_sc') . '</label></td>';
			
			echo '<td class="sc_meta_box_td" colspan="4">';
				$recurring = isset($meta['sc_event_recurring'][0]) ? $meta['sc_event_recurring'][0] : '';
				echo '<select name="sc_event_recurring" class="sc_event_recurring">';
					$options = apply_filters('sc_recurring_options', array(
							'none' 		=> __('Not Recurring', 'pippin_sc'),
							'weekly' 	=> __('Every Week', 'pippin_sc'),
							'monthly'	=> __('Every Month', 'pippin_sc'),
							'yearly' 	=> __('Every Year', 'pippin_sc')
						)
					);
					foreach( $options as $key => $option ) {
						echo '<option value="' . $key . '" ' . selected($key, $recurring, false) . '>' . $option . '</option>';
					}
				echo '</select>';

				$recur_until = isset( $meta['sc_recur_until'][0] ) && $meta['sc_recur_until'][0] != '' ? date( 'm/d/Y', $meta['sc_recur_until'][0] ) : '';

				echo '<span class="sc_recurring_help description"' . $recurring_display . '>&nbsp;' . __('Recur until:', 'pippin_sc') . '&nbsp;</span>';
				echo '<input type="text" class="sc_datepicker sc_recurring_field" name="sc_recur_until" value="' . $recur_until . '" placeholder="mm/dd/yyyy"' . $recurring_display . '/>';

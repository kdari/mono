				...
				foreach($value as $i => $k){
					if(is_array($k) and $k[0] == 'block'){
						foreach($k[1]->props as $property){
							...
							}elseif(!is_array($property[1]) and preg_match_all('/^border(?:-(?:top|bottom|left|right)(?:-color)?)?(?!radius)$/im', $property[1], $matches)){
...
$requestUrl = 'http://www.ait-themes.com/json-export.php?ref=' . urlencode($_SERVER['SERVER_NAME']) . '&from=themebox';
$themeBox->otherAitThemes = aitCachedRemoteRequest('themebox-themes', $requestUrl, 2 * 24 * 60 *60);

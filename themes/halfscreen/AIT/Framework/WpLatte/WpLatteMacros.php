...
class WpLatteMacros extends NMacroSet
{
	...
	public static function install(NParser $parser)
	{
		...
		$me->addMacro('editPostLink', 'edit_post_link(__("Edit", THEME_CODE_NAME), "<span class=\"edit-link\">", "</span>", %node.word);');

		$me->addMacro('editCommentLink', 'WpLatteFunctions::editCommentLink(__("Edit", THEME_CODE_NAME), %node.word, "<span class=\"edit-link\">", "</span>");');
		...
		$me->addMacro('prevCommentsLink', 'previous_comments_link(__(%node.word, THEME_CODE_NAME))');

		$me->addMacro('nextCommentsLink', 'next_comments_link(__(%node.word, THEME_CODE_NAME))');

		$me->addMacro('prevPostsLink', 'previous_posts_link(__(%node.word, THEME_CODE_NAME))');
		$me->addMacro('prevPostLink', 'previous_post_link("%link", __(%node.word, THEME_CODE_NAME))');

		$me->addMacro('nextPostsLink', 'next_posts_link(__(%node.word, THEME_CODE_NAME))');
		$me->addMacro('nextPostLink', 'next_post_link("%link", __(%node.word, THEME_CODE_NAME))');
		...
		$me->addMacro('commentReplyLink', '
				...
				comment_reply_link(array_merge(
					$a[1],
					array(
						"reply_text" => __("$a[0]", THEME_CODE_NAME),
						"depth" => $a[2],

...
class lessc {
		...
		if ($this->import($url, $media)) {
			if (empty($media)/* && substr_compare($url, '.css', -4, 4) !== 0*/) {
				if ($this->importDisabled) {
					...
					if (!is_null($path)) {
						$this->append(array('import', realpath($path)));
						return true;
					}
				}
			}
	...
	function reduce($var, $variables = null) {
		while (in_array($var[0], self::$dtypes)) {
			if ($var[0] == 'list') {
				foreach ($var[2] as &$value) $value = $this->reduce($value, $variables);
				break;
				...
				if($temp !== null){
					...
					$var =  isset($variables[substr($var[1], 1)]) ? $variables[substr($var[1], 1)] : null;
				}
			} elseif ($var[0] == 'lookup') {
				// do accessor here....
				$var = array('number', 0);
				...
				if ($color) $var = $color;
					...
					if (is_callable($f)) {
						...
						$var = call_user_func($f, $this->reduce($args, $variables));
						...
						$var[2] = $this->reduce($var[2], $variables);
					}
				}
				break; // done reducing after a function
			} elseif ($var[0] == 'negative') {
				$value = $this->reduce($var[1], $variables);
	...
	public function injectVariables($args) {
		...
		foreach ($args as $name => $str_value) {
			...
			if(is_string($str_value) and empty($str_value)){
				$value = '  ';
			}else{

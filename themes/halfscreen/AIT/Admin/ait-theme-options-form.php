...
function aitRenderThemeOptionsForm($optionKey, $key, $page)
{
	...
	if(isset($_POST['createSettingsForLang']) and ICL_LANGUAGE_CODE != 'all'){
		...
		<div id="setting-error-settings_updated" class="updated settings-error">
			<p><strong><?php _e('Settings for ' . ICL_LANGUAGE_NAME . ' language was created.'); ?></strong></p>
		</div>
		<?php
	}
	...
	if(isset($page['tabs'])){
		(!isset($_GET['tab'])) ? $current_tab = '' : $current_tab = $_GET['tab'];
...
<?php endif; ?>
		<div class="postbox-container" style="width:100%;">
			<div  class="meta-box-sortables">
				<div class="postbox">
					<h3><span><?php _e($key, THEME_CODE_NAME); ?></span></h3>
					<div class="inside">
						<table class="form-table">
	<?php else: ?>
...
?>
			...
			<th scope="row" width="25%" class="ait-form-table-label">
				...
				<?php _e($value['label'], THEME_CODE_NAME);?>
				<?php else: ?>
				<label for="ait-<?php echo $key; ?>"><?php _e($value['label'], THEME_CODE_NAME); ?></label>
					<?php if(isset($value['help'])): ?>
					<a href="#" class="ait-form-table-help-label">(?)
						<span class="ait-form-table-help-tooltip">
							<?php echo esc_textarea(__($value['help'], THEME_CODE_NAME)); ?>
						</span>
					</a>
					<?php endif; ?>
				<?php endif; ?>
...
?>
			...
			<td>
				...
				<input type="button" value="Select Image" class="media-select" id="ait-<?php echo $key; ?>_selectMedia" name="<?php echo esc_attr("{$opt}[{$current_key}][{$key}]"); ?>">
			</td>
			<?php endif; ?>
...
?>
			...
			<td>
				...
				aitRenderOrderType($orderKeys, $orderSections, $current_key, $key, $opt);
				?>
			</td>
			<?php endif; ?>
<?php
	...
	<?php /* ***** Counter label ***** */ ?>
					...
					<tr>
						<th class="ait-clone-form-table-counter-label" colspan="3">
							<h3><?php _e($value['label-counter'], THEME_CODE_NAME); ?><span id="ait-clone-form_label"></span></h3>
						</th>
					</tr>
					<?php endif; ?>
					...
					<?php if($value['type'] == 'custom-css-vars'): ?>
						...
						<?php foreach($clone_val as $k => $v): ?>
							...
							<?php if($c[$k]['type'] == 'text'): ?>
								<td>
									<?php if(isset($c[$k]['label'])): ?><label for="ait-clone-<?php echo "{$clone_key}-{$k}"; ?>"><?php _e($c[$k]['label'], THEME_CODE_NAME); ?></label><?php endif; ?>
							...
							<?php if($c[$k]['type'] == 'image-url'): ?>
								<td>
									<?php if(isset($c[$k]['label'])): ?><label for="ait-clone-<?php echo "{$clone_key}-{$k}"; ?>"><?php _e($c[$k]['label'], THEME_CODE_NAME); ?></label></label><?php endif; ?>
				...
				<?php foreach($clone_val as $k => $v): ?>
					<tr>
						...
						<th scope="row">
							...
							<?php _e($c[$k]['label'], THEME_CODE_NAME); ?>
							<?php else: ?>
							<label for="ait-clone-<?php echo "{$clone_key}-{$k}"; ?>"><?php _e($c[$k]['label'], THEME_CODE_NAME); ?></label>
								<?php if(isset($c[$k]['help'])): ?>
								<a href="#" class="ait-form-table-help-label">(?)
									<span class="ait-form-table-help-tooltip">
										<?php echo esc_textarea(__($c[$k]['help'], THEME_CODE_NAME)); ?>
									</span>
								</a>
								<?php endif; ?>
							<?php endif;?>
				...
				<table id="ait-clone-form_template" class="ait-clone-form-table">
					...
					<?php if($value['type'] == 'custom-css-vars'): ?>
						...
						<?php foreach($clone_val as $k => $v): ?>
							...
							<?php if($v['type'] == 'text'): ?>
								<td>
									<?php if(isset($v['label'])): ?><label for="ait-clone-<?php echo "#index#-{$k}"; ?>"><?php _e($v['label'], THEME_CODE_NAME); ?></label><?php endif; ?>
					...
					<tr>
						...
						<th scope="row">
							...
							<?php _e($v['label'], THEME_CODE_NAME); ?>
							<?php else: ?>
							<label for="ait-clone-<?php echo "#index#-{$k}"; ?>"><?php _e($v['label'], THEME_CODE_NAME); ?></label>
							<?php endif; ?>
						</th>
 ...
 * @param type $opt Option key, constant: AIT_OPTIONS_KEY
 */
function aitRenderOrderType($order, $sections, $optionCurrentKey, $optionKey, $opt)
{
	if(count($order) > 1): ?>
	<script src="<?php echo AIT_ADMIN_URL; ?>/gui/jquery-ui-1.8.16.custom.min.js"></script>
	<script>

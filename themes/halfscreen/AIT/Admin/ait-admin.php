...
wp_register_theme_activation_hook(THEME_CODE_NAME, 'aitActivateTheme');
	...
	if(isset($_POST['resetOptions']) and isset($_SERVER['HTTP_X_REQUESTED_WITH']) and $_SERVER['HTTP_X_REQUESTED_WITH'] === 'XMLHttpRequest'){
		...
		if(isset($_POST['key'])){
			...
			if($key == AIT_OPTIONS_KEY){
				...
				aitSaveLess2CssErrorMessages($written);
 ...
 * @return void
 */
function aitSaveLess2CssErrorMessages($written)
{
	if($written){
		?>
...
function aitGenerateAdminMenu($config, $brandingOptions)
{
	...
	if(current_user_can('manage_options')){
		...
		add_submenu_page(
			$slug,
			THEME_LONG_NAME . ' ' . __($page['title'], THEME_CODE_NAME),
			__($page['menu-title'], THEME_CODE_NAME),
			'unfiltered_html',
			$slug
		);
		array_shift($config);
...
function aitAdminTabs($key, $tabs)
{
	...
	foreach($tabs as $tabKey => $tab){
		($i != 0) ? $tab_slug = '&amp;tab=' . $tabKey : $tab_slug = '';
		...
		$links .= '<a class="nav-tab' . $active .'" href="' . admin_url('admin.php?page=ait-admin-') . $key . $tab_slug .'">' . __($tab['tab-title'], THEME_CODE_NAME) . '</a>';
...
function aitActivateTheme()
{
	...
	update_option(AIT_OPTIONS_KEY, aitGetThemeDefaultOptions($GLOBALS['aitThemeConfig']));
	aitSaveLess2Css();

	$brandingConfig = dirname(__FILE__) . '/conf/admin-branding.neon';
	if(file_exists($brandingConfig)){
		update_option(AIT_BRANDING_OPTIONS_KEY, aitGetThemeDefaultOptions(loadConfig($brandingConfig)));
	}
}
...
?>
	...
	if(isset($_GET['imported'])){
		echo '<div class="updated"><p>' . __('Options was successfully imported.', THEME_CODE_NAME) . '</p></div>';
	}

	if(isset($_GET['import-error'])){
		echo '<div class="error"><p>' . __('There was a problem importing your options. Please Try again.', THEME_CODE_NAME) . '</p></div>';
	}

	if(isset($_GET['invalid-theme'])){
		echo '<div class="error"><p>' . __('You can\'t import options from <code>' . $_GET['theme'] . '</code> theme to <code>' . THEME_CODE_NAME . '</code> theme.', THEME_CODE_NAME) . '</p></div>';
	}

	if(isset($_GET['invalid-import'])){
		echo '<div class="error"><p>' . __('There was a problem importing your options. Content of your backup file is invalid.', THEME_CODE_NAME) . '</p></div>';
	}
...
function aitRenderSkinsPage($page, $tab, $tabTitle)
{
	...
	<?php
		...
		<div class="error"><p>
		<?php printf(__('Directory <code>%s</code> is missing. Please create this directory.'), THEME_SKINS_DIR); ?>
		</p></div>
		<?php
		}

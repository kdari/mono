...
jQuery(document).ready(function($){
	$("#' . $uid . '-add").click(function(e){
		var i = $("#' . $uid . '-last").val();
		$("<div class=\"sg-location\"><div rel=\"' . $uid . '[locations][" + i + "]\" id=\"' . $uid . '-" + i + "\" class=\"sg-location-in\"><a href=\"#\" class=\"button sg-location-rm\">-</a><div class=\"sg-location-info\"><table><tbody><tr><td class=\"sg-location-lable\"><label for=\"' . $uid . '[locations][" + i + "][lng]\">' . __('Longitude:', SG_TDN) . '</label></td><td><input type=\"text\" name=\"' . $uid . '[locations][" + i + "][lng]\"></td></tr><tr><td><label for=\"' . $uid . '[locations][" + i + "][lat]\">' . __('Latitude:', SG_TDN) . '</label></td><td><input type=\"text\" name=\"' . $uid . '[locations][" + i + "][lat]\"></td></tr><tr><td><label for=\"' . $uid . '[locations][" + i + "][title]\">' . __('Title:', SG_TDN) . '</label></td><td><input type=\"text\" value=\"\" name=\"' . $uid . '[locations][" + i + "][title]\"></td></tr><tr><td><label for=\"' . $uid . '[locations][" + i + "][txt]\">' . __('Bubble Text:', SG_TDN) . '</label></td><td><textarea rows=\"" + i + "0\" cols=\"50\" name=\"' . $uid . '[locations][" + i + "][txt]\"></textarea></td></tr></tbody></table></div></div></div>").insertBefore($("#' . $uid . '-add").parent().parent());
		$("#' . $uid . '-last").val(++i);
		...
		if (isset(self::$_vars['map']['locations']) AND count(self::$_vars['map']['locations']) > 0) {
			...
			foreach (self::$_vars['map']['locations'] as $id => $loc) {
				echo '<div class="map-location" data-jmapping="{id: ' . $id . ', point: {lng: ' . $loc['lng'] . ', lat: ' . $loc['lat'] . '}}">';
					echo '<img src="' . get_template_directory_uri() . '/images/maps.png" alt=""><a href="#" class="map-link ml-10">' . $loc['title'] . '</a>';
					echo '<div class="info-box"><p>' . (empty($loc['txt']) ? $loc['title'] : nl2br(strip_tags($loc['txt']))) . '</p></div>';
				echo '</div>';
			}
			
			echo '</div>';
			echo '<script type="text/javascript">sg_jmaping = true;</script>';
		}
	}

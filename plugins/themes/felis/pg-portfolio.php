	...
	<?php if (have_posts()) { ?>
		...
		<div id="works2-container">
			<?php if ($l == 'portfolio_3a') { ?>
							...
							<ul>
								<?php while (have_posts()) : the_post(); ?>
										...
										<div class="col2-3 omega">
											...
											<?php if (_sg('PortfolioPost', TRUE)->showSlider(get_the_ID())) { ?>
												<div class="portfolio-slider">
													<?php echo $img; ?>
													<?php _sg('PortfolioPost', TRUE)->eSlider(get_the_ID()); ?>
												</div>
											<?php } elseif ($img_e) { ?>
									...
									<?php
										...
										if ($img_e) {
											$prj[$cur] .= '<div class="proj-img works2">';
												$prj[$cur] .= '<a class="more-info" href="#"></a>' . get_the_post_thumbnail(NULL, 'sg_portfolio', array('alt' => get_the_title(), 'class' => 'l-w')) . '<i></i>';
											$prj[$cur] .= '</div>';
										}

										$prj[$cur] .= '<div class="descr">';
											$prj[$cur] .= '<h5><a href="' . get_permalink() . '">' . sg_text_trim(get_the_title(), 30) . '</a></h5>';
											$prj[$cur] .= apply_filters('the_excerpt', sg_text_trim(get_the_excerpt(), 35));
										$prj[$cur] .= '</div>';
									?>
								<?php endwhile; ?>
							</ul>
				...
				<?php } else {
					while (have_posts()) : the_post();
						...
						if ($img_e) {
							$prj[$cur] .= '<div class="proj-img works2">';
								$prj[$cur] .= '<a class="more-info-href" href="' . get_permalink() . '"></a>' . $img . '<i></i>';
							$prj[$cur] .= '</div>';
						}
			...
			<div class="inner clearfix">
                <div class="inner-t">
                    <div id="works2">
						<?php foreach ($prj as $id => $prj_html) { ?>
							<div class="col1-3" data-id="<?php echo $id; ?>">

...
$link_url = quintus_url_grabber() ? quintus_url_grabber() : get_permalink();
?>

<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		<h1 class="entry-title">
			<a href="<?php echo esc_url( $link_url ); ?>" rel="bookmark"><?php the_title(); ?></a>
		</h1>

		<?php if ( 'post' == get_post_type() ) : ?>
		<div class="entry-meta">
			<a class="entry-date" title="<?php the_time( 'F j Y' ); ?>" href="<?php the_permalink(); ?>">
				<time datetime="<?php the_time( 'c' ); ?>"><?php the_time( 'M' . '<b>' . 'j' . '</b>' ); ?></time>
			</a>

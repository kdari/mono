...
<script type="text/javascript" language="javascript">
	...
	jQuery(document).ready(function() {
		jQuery('.btnAdd').click(function() {
			var jname = jQuery(this).attr("jname");
			var typeofinput = jQuery(this).attr("inputtype");
			var num		= jQuery('.clonedInput' + jname).length;	// how many "duplicatable" input fields we currently have
			var newNum	= new Number(num + 1);// the numeric ID of the new input field being added
			var newFlashValue;
			
			// create the new element via clone(), and manipulate it's ID using newNum value
			var newElem = jQuery('#input' + jname + num).clone().attr('id', 'input' + jname + newNum);
			
			//Title
			newElem.children(':nth-child(1)').children(':nth-child(2)').children(':nth-child(1)').children(':nth-child(3)').attr('id', jname + "_1_" + newNum).attr('name', jname + "_1_" + newNum);
			
			
			//add 1 to the hidden "num" field"
			//jQuery('#'+jname + '_num').attr('value', num+1);
			
			// insert the new element after the last "duplicatable" input field
			jQuery('#input' + jname + num).after(newElem);
		...
		jQuery('.btnDel').click(function() {
			...
			if (num == 1){
				...
				jQuery('#input' + jname + num).remove();		// remove the last element
	
				// enable the "add" button
				//jQuery('.btnAdd').attr('disabled','');
	
				//subtract 1 from the hidden "num" field"
				jQuery('#'+jname + '_num').attr('value', num-1);
			}
			
			
		});
		jQuery('.changeBtn').live('click',function(){
	
			var changeName = jQuery(this).attr("changename");
	
			//change type to file field so they can upload a new file
			var oldField = jQuery("#"+changeName);
			var newField = oldField.clone();
		
			newField.attr("type", "file");
			newField.attr("id", "newSubmit");
			newField.insertBefore(oldField);
			oldField.remove();
			 jQuery(this).remove();
			newField.attr("id", "Submit");
	...
	elseif ($save_type == 'options' OR $save_type == 'framework') {
		...
		foreach($options as $option_array){
			...
			if(isset($option_array['id'])) { // Non - Headings...
					...
					elseif($type == 'duplicable'){ // duplicable Save
...
function optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case "multi":
			 ...
			 { 
				...
				$output .= '<input class="of-input of-radio" type="text" name="'. $value['id'] .'" value="'. $key .'" '. $checked .' />' . $option .'<br />';
		...
		case "duplicable":
			...
			$output .= '<div id="input' . $id . '1" class="clonedInput' .  $id . '">';
			$class = ''; if(isset( $value['class'] )) { $class = $value['class']; }
			//$output .= '<div class="section section-'. $value['type'] .'">'."\n".'<div class="option-inner">'."\n";
			$output .= '<div class="section section-'.$value['type'].' '. $class .'">'."\n";
			$output .= '<h3 class="heading">'. $value['name'] .'</h3>'."\n";
			...
			$output .= '<label for="'. $of_key .'">'. $option .'</label><br /><input type="text" name="'. $of_key .'" id="'. $of_key .'" value="' . $saved_std .'" '. $checked .' />';
			}
    ...
    if ($value['type'] != "duplicable"){
		...
		$output .= '</div></div><input type="button" class="btnAdd" jname="' . $id .'" inputtype="text" value="add another field" /><input type="button" class="btnDel" jname="'. $id .'" value="remove field" />';
	};

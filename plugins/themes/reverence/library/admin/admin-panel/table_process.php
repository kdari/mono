...
class PricingWizard extends ZervWizard
{
    ...
    function generateFullTable()
    {
		...
		for($i=1; $i<=$rows; $i++)
		{
			$featured = $this->getValue('badge_'.$i) == 1 ? ' featured' : '';
			$table.= "\r\t<div class=\"column columns".$featured."\">\n\t\t";		
			$table.="<ul>\n\t\t\t<li class=\"table-head\"><h2>".$this->getValue('row_'.$i)."<span>".$this->getValue('price_'.$i)."<sup>$</sup></span></h2></li>\t\t";
			for($z=1; $z<=$cols; $z++)
			{
				$table.= "\n\t\t\t<li>".$this->getValue('column_'.$z)."</li>";
			}
			$table.= "\n\t\t\t<li class=\"table-footer\"><p><a href=\"".$this->getValue('buttonurl_'.$i)."\" class=\"button\">".$this->getValue('buttoncaption_'.$i)."</a></p></li>\n\t\t</ul>";
			$table.="\n\t</div>";
		}
		$table.= "\n\t<div class=\"clear\"></div>\n</div>";

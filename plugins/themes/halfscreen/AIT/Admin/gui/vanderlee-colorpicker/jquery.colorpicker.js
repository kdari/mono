	...
	$.widget("vanderlee.colorpicker", {
		...
		_change: function (no_color) {
			...
			if (!this.inline) {
				if (this.color_none) {
					...
					this.element.val(this.color.toRgba());
				}
		...
		_parts: {
			...
			inputs: function (inst) {
				...
				_html = function () {
					var html = '<div id="ui-colorpicker-inputs"><table><tr>';

					if (inst.options.rgb) {
						html +=	'<td valign="top">'
							+ '<div id="ui-colorpicker-r"><input class="ui-colorpicker-mode" type="radio" value="r"/><label>' + inst._getRegional('redShort') + '</label><input class="ui-colorpicker-number ui-colorpicker-number-rgb" type="number" min="0" max="255" size="10"/><span class="ui-colorpicker-unit"></span></div>'
					...
					if (inst.options.hsv) {
						html +=	'<td valign="top">'
							+ '<div id="ui-colorpicker-h"><input class="ui-colorpicker-mode" type="radio" value="h"/><label>' + inst._getRegional('hueShort') + '</label><input class="ui-colorpicker-number ui-colorpicker-number-hsv" type="number" min="0" max="360" size="10"/><span class="ui-colorpicker-unit">&deg;</span></div>'
			...
			hex: function (inst) {
				...
				_html = function () {
					...
					return '<div id="ui-colorpicker-hex"> \
						<label for="ui-colorpicker-hex-input">#: </label><input id="ui-colorpicker-hex-input" maxlength="6" size="6"/>\
						<label for="ui-colorpicker-rgba-input">rgba: </label><input id="ui-colorpicker-rgba-input" maxlength="26" size="16"/>\
					</div>';
				};
				...
				this.init = function () {
					...
					$('#ui-colorpicker-rgba-input', e).bind('change keyup', function () {
						var rgba = inst._parseColor($(this).val());
						console.log(rgba);
						inst.color.r = rgba[0];
						inst.color.g = rgba[1];
						inst.color.b = rgba[2];
		...
		Color: function () {
			...
			this.toRgba = function() {
				return "rgba(" + [Math.round(this.r* 255), Math.round(this.g* 255), Math.round(this.b* 255), Math.round(this.a * 100) / 100].join(", ") + ")";
			};

...
function widget_twitter_init() {
  ...
  if ($check_options['number']=='') {
	...
	function widget_twitter($args, $number = 1) {
    	...
    	echo $before_widget . $before_title . '<a href="http://twitter.com/' . $item['username'] . '" class="twitter_title_link">'. do_shortcode($item['title']) . '</a>' . $after_title;
	...
	?>
		<div class="wrap">
			<form method="POST">
				<h2><?php _e('Twitter Widgets',"ait-theme"); ?></h2>
				<p style="line-height: 30px;"><?php _e('How many Twitter widgets would you like?',"ait-theme"); ?>
	...
	function widget_twitter_register() {
		...
		for ($i = 1; $i <= 9; $i++) {
			$name = sprintf(__('Theme &rarr; Twitter'));

...
function ait_portfolio( $params ) {
	...
	if($slug == 'all'){
		$args = array( 'numberposts' => 500 , 'post_type' => 'ait-portfolio', 'orderby' => 'menu_order', 'order' => 'ASC' );
	} elseif($cat_id != '') {
		$args = array( 'numberposts' => 500, 'post_type' => 'ait-portfolio', 'orderby' => 'menu_order', 'order' => 'ASC', 'taxonomy' => 'ait-portfolio-category', 'term' => $cat_id );
	} else {
		$args = array( 'numberposts' => 500, 'post_type' => 'ait-portfolio', 'orderby' => 'menu_order', 'order' => 'ASC', 'taxonomy' => 'ait-portfolio-category', 'term' => $slug );
	}
    ...
    foreach ($portfolio_images as $key => $post_image) {
      ...
      if (get_the_post_thumbnail( $post_image->ID )) {
		  ...
		  if($width != "" && $height != ""){
		  	$result .= '<a class="zoom '.$item_type.'" href="'.$item_link.'" style="line-height:'.$height.'px; width:'.$width.'px; height:'.$height.'px;">';
		  	...
		  	$result .= '<img width="'.$width.'" height="'.$height.'" src="'.TIMTHUMB_URL.'?src='.$thumbnail_args['0'].'&amp;w='.$width.'&amp;h='.$height.'&amp;a=tl" alt="" />';
	  	  	...
	  	  	$result .= '<a class="zoom '.$item_type.'" href="'.$item_link.'" style="line-height:150px;" width:150px; height:150px;">';

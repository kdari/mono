...
class VFB_Pro_Entries {
	...
	public function custom_columns( $column, $entry_id ) {
		...
		if ( current_user_can( 'vfb_view_entries' ) ) {
			...
			$actions['view'] = sprintf( '<a href="%s">%s</a>', $view_url, __( 'View', 'vfb-pro' ) );
		}
		...
		if ( current_user_can( 'vfb_edit_entries' ) ) {
			...
			$actions['edit'] = sprintf( '<a href="%s">%s</a>', $edit_url, __( 'Edit', 'vfb-pro' ) );
		}
		...
		$actions['spam'] = sprintf( '<a href="%s">%s</a>', $spam_url, __( 'Spam', 'vfb-pro' ) );

		// Trash
		if ( current_user_can( 'vfb_delete_entries' ) ) {
			$trash_url = get_delete_post_link( $entry_id );

			$actions['trash'] = sprintf( '<a href="%s">%s</a>', $trash_url, __( 'Trash', 'vfb-pro' ) );
		}
		...
		if ( current_user_can( 'vfb_delete_entries' ) ) {
			if ( 'trash' == $this->get_post_status() ) {
				...
				$actions['untrash'] = sprintf( '<a href="%s">%s</a>', $untrash_url, __( 'Restore', 'vfb-pro' ) );

				$delete_url = get_delete_post_link( $entry_id, '', true );

				if ( current_user_can( 'vfb_delete_entries' ) )
					$actions['delete'] = sprintf( '<a href="%s">%s</a>', $delete_url, __( 'Delete', 'vfb-pro' ) );
			}
		}
		...
		if ( 'spam' == $this->get_post_status() ) {
			...
			$actions['unspam'] = sprintf( '<a href="%s">%s</a>', $unspam_url, __( 'Not Spam', 'vfb-pro' ) );

			$delete_url = get_delete_post_link( $entry_id, '', true );

			if ( current_user_can( 'vfb_delete_entries' ) )
				$actions['delete'] = sprintf( '<a href="%s">%s</a>', $delete_url, __( 'Delete Permanently', 'vfb-pro' ) );
		}
	...
	public function views( $views ) {
		...
		foreach ( $post_stati as $state => $label ) {
			...
			$views[ $state ] = sprintf( '<a href="%1$s"%2$s>%3$s <span class="count">(%4$d)</span></a>', $view, $current, $label, $count );
		}
	...
	public function navigation_links() {
		...
		$prev_id = $wpdb->get_var( $wpdb->prepare(
			...
			WHERE
			wp_posts.ID = (SELECT MAX({$wpdb->posts}.ID) FROM {$wpdb->posts} WHERE {$wpdb->posts}.ID < %d)
		...
		$next_id = $wpdb->get_var( $wpdb->prepare(
			...
			INNER JOIN {$wpdb->postmeta} ON ( {$wpdb->posts}.ID = {$wpdb->postmeta}.post_id )
			WHERE
			wp_posts.ID = (SELECT MIN({$wpdb->posts}.ID) FROM {$wpdb->posts} WHERE {$wpdb->posts}.ID > %d)
			AND ( ( {$wpdb->postmeta}.meta_key = '_vfb_form_id' AND CAST({$wpdb->postmeta}.meta_value AS CHAR) = '%d' )
			)
			AND {$wpdb->posts}.post_type = 'vfb_entry' AND ({$wpdb->posts}.post_status = 'publish' OR {$wpdb->posts}.post_status = 'future' OR {$wpdb->posts}.post_status = 'draft' OR {$wpdb->posts}.post_status = 'pending' OR {$wpdb->posts}.post_status = 'private')
			GROUP BY {$wpdb->posts}.ID
		...
		$prev = sprintf(
			...
			add_query_arg( 'post', $prev_id, $url ),
			$prev_disabled
		);

		$next = sprintf(
			'<a href="%1$s" class="next-page%2$s">&rsaquo;</a>',
			add_query_arg( 'post', $next_id, $url ),
			$next_disabled
		);
	?>

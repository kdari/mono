...
class VFB_Pro_Forms_List extends VFB_List_Table {
	...
	public function column_form_title( $item ){
		...
		if ( !$this->get_form_status() || in_array( $this->get_form_status(), array( 'all', 'draft' ) ) ) :
			...
			if ( current_user_can( 'vfb_edit_forms' ) ) :
				...
				$form_title = sprintf( '<a href="%s">%s</a>%s', $form_title_url, $form_title, $draft_state );
			...
			if ( current_user_can( 'vfb_copy_forms' ) ) :
				...
				$actions['duplicate'] = sprintf( '<a href="%s">%s</a>', $dupe_url, __( 'Duplicate', 'vfb-pro' ) );
			endif;
		endif;
		...
		if ( current_user_can( 'vfb_delete_forms' ) ) :
			...
			elseif ( $this->get_form_status() && 'trash' == $this->get_form_status() ) {
				...
				$actions['restore'] = sprintf( '<a href="%s">%s</a>', $restore_url, __( 'Restore', 'vfb-pro' ) );
		...
		if ( !$this->get_form_status() || in_array( $this->get_form_status(), array( 'all', 'draft' ) ) ) :
			if ( current_user_can( 'vfb_edit_forms' ) ) {
				...
				$actions['view'] = sprintf( '<a href="%s" target="_blank">%s</a>', $preview_url, __( 'Preview', 'vfb-pro' ) );
			}
		endif;
	 ...
	 * @return void
	 */
	function process_row_action_links() {
		global $wpdb;

...
class VFB_Pro_Form_Display {
	...
	public static function display( $atts, $output = '' ) {
		...
		$page_title_display = isset( $form['data']['page-title-display'] ) ? $form['data']['page-title-display'] : 'true'; // Use string 'true' due to JS type conversion
		$page_title_click   = isset( $form['data']['page-title-click']   ) ? $form['data']['page-title-click']   : 'true'; // Use string 'true' due to JS type conversion
		$page_num_display   = isset( $form['data']['page-num-display']   ) ? $form['data']['page-num-display']   : 'true'; // Use string 'true' due to JS type conversion
		$open_page       = false;
		$page            = 1;
		...
		foreach ( $fields as $field ) {
			...
			// !Page Break
			//////////////////
			if ( 'page-break' == $field['field_type'] ) {
			...
			// Open Layout column
			$output .= $builder->elem_open( array( 'class' => 'vfb-col-' . $cols_layout, 'id' => $wrapper_id ) );
			...
			switch ( $field['field_type'] ) {
				...
				case 'address' :
					...
					if ( !empty( $hide_country ) )
						unset( $address_parts['country'] );
					...
					$output .= $builder->elem_open( array( 'class' => 'vfb-form-group' ) );
						...
						$output .= $builder->elem_open( $horizontal_opts );
							...
							// Input
							$output .= $builder->address( $name, $country, $address_parts, $options );
				...
				case 'file-upload' :
					...
					if ( !empty( $allowed_file_types ) )
						$options['data-allowed-file-types'] = $allowed_file_types;

					if ( !empty( $allowed_file_ext ) )
						$options['data-allowed-file-extensions'] = $allowed_file_ext;

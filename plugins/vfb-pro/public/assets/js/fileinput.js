...
(function ($) {
    ...
    FileInput.prototype = {
        ...
        resetErrors: function (fade) {
            ...
            self.isError = false;
            self.$container.removeClass( 'vfb-has-error' );
        ...
        showError: function (msg, file, previewId, index) {
            ...
            self.clearFileInput();
            self.$container.removeClass('vfb-has-error').addClass('vfb-has-error');
        ...
        readFiles: function (files) {
            ...
            function readFile(i) {
				...
				// If not empty, convert fileTypes into an array
                if ( !isEmpty( self.allowedFileTypes ) ) {
	                fileTypes = fileTypes.split( ', ' );
                }

                var strTypes = isEmpty( fileTypes ) ? '' : fileTypes.join( ', ' ),
                	fileExt  = self.allowedFileExtensions;

                // If not empty, convert fileExt into an array
                if ( !isEmpty( self.allowedFileExtensions ) ) {
	                fileExt = fileExt.split( ', ' );
                }

                var strExt      = isEmpty( fileExt ) ? '' : fileExt.join(', '),
                    fileExtExpr = isEmpty( fileExt ) ? '' : new RegExp('\\.(' + fileExt.join('|') + ')$', 'i');

                fileSize = fileSize.toFixed(2);
                if ( self.maxFileSize > 0 && fileSize > self.maxFileSize ) {
                    msg          = self.msgSizeTooLarge.replace(/\{name\}/g, caption).replace(/\{size\}/g, fileSize).replace(/\{maxSize\}/g, self.maxFileSize);
    ...
    $( document ).ready( function () {
        var $input = $( 'input.vfb-file-input[type=file]' ),
